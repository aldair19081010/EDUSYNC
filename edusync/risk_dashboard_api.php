...
try{
    ...
    $filters=[];
    foreach(['level','grade','section','bimestre','search','risk_level','intervention_status','course_id'] as $key){
    ...
    if($method==='GET'){
        if($action==='dashboard'){
            risk_api_reply(risk_api_normalize_course_dashboard(edu_course_dashboard_data($conn,$actor,$filters)));
        }
        if($action==='student'){
        ...
        if($action==='interventions'){
            $f=[];
            foreach(['level','grade','section'] as $key)if(!empty($filters[$key]))$f[$key]=$filters[$key];
            if(!empty($filters['intervention_status']))$f['status']=$filters['intervention_status'];
    ...
    if($action==='create_intervention'){
        $result=edu_course_create_intervention($conn,$actor,$_POST);
        risk_api_reply($result,!empty($result['ok'])?200:422);
    }
    if($action==='update_intervention'){
        $id=(int)($_POST['id']??0);
        if($id<=0)risk_api_reply(['ok'=>false,'message'=>'Intervención no válida.'],422);
        $result=edu_course_update_intervention($conn,$actor,$id,$_POST);
        risk_api_reply($result,!empty($result['ok'])?200:422);
    }
    risk_api_reply(['ok'=>false,'message'=>'Acción no válida.'],404);
}catch(Throwable $e){

...
function edu_chat_adv_finance_by_amount_result(mysqli $conn,array $actor,array $entities,float $minDebt=0.01,?float $maxDebt=null,bool $overdueOnly=false,int $limit=100): array {
    ...
    $lines=[];$sum=0.0;foreach($rows as $i=>$r){$sum+=(float)$r['total_debt'];$loc=$r['nivel'].' · '.edu_chat_grade_label($r['grado']).' '.$r['seccion'];$extra=$overdueOnly?' · '.(int)$r['overdue_obligations'].' vencida'.((int)$r['overdue_obligations']===1?'':'s'):'';$lines[]=($i+1).'. '.$r['name'].' — '.edu_chat_money((float)$r['total_debt']).' · '.(int)$r['obligations'].' deuda'.((int)$r['obligations']===1?'':'s').$extra.' · '.$loc;}
...
function edu_chat_adv_attendance_rate_result(mysqli $conn,array $actor,array $entities,float $maxRate,int $limit=100): array {
    ...
    $lines=[];foreach($rows as $i=>$r)$lines[]=($i+1).'. '.$r['name'].' — '.number_format((float)$r['rate'],1).'% · '.(int)$r['absent'].' ausencias · '.(int)$r['late'].' tardanzas · '.$r['nivel'].' · '.edu_chat_grade_label($r['grado']).' '.$r['seccion'];
...
function edu_chat_adv_missing_entry_result(mysqli $conn,array $actor,array $entities,int $limit=100): array {
    ...
    $lines=[];foreach($rows as $i=>$r)$lines[]=($i+1).'. '.$r['name'].' — '.$r['nivel'].' · '.edu_chat_grade_label($r['grado']).' '.$r['seccion'];
...
function edu_chat_adv_academic_courses_result(mysqli $conn,array $actor,array $entities,int $minCourses=1,int $limit=100): array {
    ...
    $lines=[];foreach($rows as $i=>$r)$lines[]=($i+1).'. '.$r['name'].' — '.(int)$r['critical_courses'].' curso'.((int)$r['critical_courses']===1?'':'s').' con riesgo · '.(int)$r['critical_records'].' registros críticos · '.$r['nivel'].' · '.edu_chat_grade_label($r['grado']).' '.$r['seccion'].(!empty($r['courses'])?' · '.$r['courses']:'');

 ...
 * Las simulaciones describen cambios dentro del modelo; no representan causalidad
 * ni garantizan que una intervención produzca el resultado estimado.
 */
...
function edu_risk_student_by_id(mysqli $conn,array $actor,int $studentId): ?array {
    ...
    if(!$stmt)return null;
    $stmt->bind_param('ii',$studentId,$schoolId);$stmt->execute();$row=$stmt->get_result()->fetch_assoc();$stmt->close();
    return $row?:null;
}
...
function edu_risk_statuses(): array {
    return ['Pendiente','En proceso','Completada','Cancelada'];
}

function edu_risk_outcomes(): array {
    return ['Mejoró','Sin cambio','Empeoró','No evaluado'];
}

function edu_risk_counterfactual_apply(array $base,array $changes): array {
    $f=$base;
    foreach($changes as $feature=>$value)$f[$feature]=$value;
    if(isset($changes['grade_mean_current'])){
        $previous=(float)($f['grade_mean_previous']??$f['grade_mean_current']);
        $f['grade_trend']=(float)$f['grade_mean_current']-$previous;
    }
    return $f;
}
...
function edu_risk_counterfactual_scenarios(array $prediction): array {
    if(empty($prediction['available'])||empty($prediction['model']))return [];
    $base=(array)$prediction['features'];$model=(array)$prediction['model'];
    $current=(float)$prediction['probability'];
    $medium=(float)($model['risk_thresholds']['medium']??0.40);$high=(float)($model['risk_thresholds']['high']??0.70);
    $target=$prediction['level']==='Alto'?$high:($prediction['level']==='Medio'?$medium:0.0);
    if($prediction['level']==='Bajo')return [];

    $mean=(float)($base['grade_mean_current']??0);$critRecords=(float)($base['critical_records_current']??0);$critCourses=(float)($base['critical_courses_current']??0);
    $attendance=(float)($base['attendance_rate_30d']??100);$late=(float)($base['late_30d']??0);$absent=(float)($base['absent_30d']??0);

    $templates=[
        ['name'=>'Mejora de asistencia','effort'=>1,'changes'=>[
            'attendance_rate_30d'=>max($attendance,90.0),
            'late_30d'=>floor($late/2),
            'absent_30d'=>floor($absent/2)
        ]],
        ['name'=>'Refuerzo académico focalizado','effort'=>2,'changes'=>[
            'grade_mean_current'=>min(20.0,$mean+1.5),
            'critical_records_current'=>max(0.0,$critRecords-2),
            'critical_courses_current'=>max(0.0,$critCourses-1)
        ]],
        ['name'=>'Asistencia sostenida','effort'=>3,'changes'=>[
            'attendance_rate_30d'=>max($attendance,95.0),
            'late_30d'=>0.0,
            'absent_30d'=>0.0
        ]],
        ['name'=>'Plan combinado moderado','effort'=>4,'changes'=>[
            'attendance_rate_30d'=>max($attendance,92.0),
            'late_30d'=>floor($late/2),
            'absent_30d'=>floor($absent/2),
            'grade_mean_current'=>min(20.0,$mean+1.5),
            'critical_records_current'=>max(0.0,$critRecords-2),
        ...
        ['name'=>'Plan combinado intensivo','effort'=>5,'changes'=>[
            'attendance_rate_30d'=>max($attendance,95.0),
            'late_30d'=>0.0,
            'absent_30d'=>0.0,
            'grade_mean_current'=>min(20.0,$mean+2.5),
            'critical_records_current'=>max(0.0,$critRecords-4),
            'critical_courses_current'=>max(0.0,$critCourses-2)
        ]]
    ];
    ...
    foreach($templates as $template){
        ...
        $rows[]=[
            'name'=>$template['name'],'effort'=>$template['effort'],'probability'=>$after,'level'=>$score['level'],
            'reduction'=>$current-$after,'crosses_target'=>$after<$target,'features'=>$features,
            'changes'=>edu_risk_counterfactual_change_items($base,$features)
        ];
    }
...
function edu_risk_suggest_intervention(array $prediction): array {
    $raises=(array)($prediction['explanation']['raises']??[]);$features=[];
    foreach($raises as $item)$features[(string)($item['feature']??'')]=(float)($item['impact']??0);
...
function edu_risk_create_intervention(mysqli $conn,array $actor,array $data): array {
    ...
    foreach([&$planned,&$followup] as &$d){if($d!==''&&!preg_match('/^\d{4}-\d{2}-\d{2}$/',$d))return ['ok'=>false,'message'=>'La fecha indicada no tiene un formato válido.'];}unset($d);
    ...
    $stmt->bind_param('iiiiidssssssii',$schoolId,$yearId,$studentId,$source,$target,$probability,$level,$type,$title,$notes,$planned,$followup,$userId,$userId);
...
function edu_risk_update_intervention(mysqli $conn,array $actor,int $id,array $data): array {
    ...
    $stmt->bind_param('ii',$id,$schoolId);$stmt->execute();$row=$stmt->get_result()->fetch_assoc();$stmt->close();if(!$row)return ['ok'=>false,'message'=>'No encontré la intervención solicitada.'];
    ...
    $followup=$followup!==''?$followup:null;$outcome=$outcome!==''?$outcome:null;$post=null;$completed=null;
    if($status==='Completada'){
        $completed=date('Y-m-d H:i:s');$p=edu_predictive_student_prediction($conn,$actor,(int)$row['student_id'],null);if(!empty($p['available']))$post=(float)$p['probability'];
        if($outcome===null)$outcome='No evaluado';
    }
    $userId=edu_risk_actor_user_id($actor);
    $sql='UPDATE student_risk_interventions SET status=?,outcome=?,notes=?,followup_date=?,completed_at=COALESCE(?,completed_at),post_risk_probability=COALESCE(?,post_risk_probability),updated_by=? WHERE id=? AND school_id=?';
    $stmt=$conn->prepare($sql);if(!$stmt)return ['ok'=>false,'message'=>'No pude preparar la actualización.'];
    $stmt->bind_param('sssssdi ii',$status,$outcome,$notes,$followup,$completed,$post,$userId,$id,$schoolId);
    // mysqli no admite espacios en la cadena de tipos; se enlaza de nuevo de forma segura.
    $stmt->close();
    $stmt=$conn->prepare($sql);if(!$stmt)return ['ok'=>false,'message'=>'No pude preparar la actualización.'];
...
function edu_risk_list_interventions(mysqli $conn,array $actor,array $filters=[],int $limit=100): array {
    if(!edu_risk_interventions_ready($conn))return [];$schoolId=(int)($actor['school_id']??0);$limit=max(1,min(300,$limit));
    $where=['ri.school_id=?'];$types='i';$params=[$schoolId];
...
function edu_risk_open_counts_by_student(mysqli $conn,array $actor): array {
    if(!edu_risk_interventions_ready($conn))return [];$schoolId=(int)($actor['school_id']??0);
    $stmt=$conn->prepare("SELECT student_id,COUNT(*) total FROM student_risk_interventions WHERE school_id=? AND status IN ('Pendiente','En proceso') GROUP BY student_id");if(!$stmt)return[];
    $stmt->bind_param('i',$schoolId);$stmt->execute();$res=$stmt->get_result();$map=[];while($r=$res->fetch_assoc())$map[(int)$r['student_id']]=(int)$r['total'];$stmt->close();return$map;
}
...
function edu_risk_dashboard_data(mysqli $conn,array $actor,array $entities=[]): array {
    ...
    if(!empty($model['available'])){
        foreach($students as $student){
            ...
            if(empty($p['available']))continue;$evaluated++;$summary[$p['level']]++;
            foreach(array_slice((array)($p['explanation']['raises']??[]),0,3) as $factor){$label=(string)($factor['label']??$factor['feature']??'Factor');$factorCounts[$label]=($factorCounts[$label]??0)+1;}
            $predictions[]=['student'=>$student,'probability'=>(float)$p['probability'],'level'=>$p['level'],'bimester'=>$p['bimester'],'target_bimester'=>$p['target_bimester'],'factors'=>array_slice((array)($p['explanation']['raises']??[]),0,3),'open_interventions'=>$openMap[(int)$student['id']]??0,'suggested'=>edu_risk_suggest_intervention($p)];
        }
    ...
    $interventions=edu_risk_list_interventions($conn,$actor,$entities,100);$intSummary=['Pendiente'=>0,'En proceso'=>0,'Completada'=>0,'Cancelada'=>0,'Seguimientos vencidos'=>0];$improved=0;$measured=0;$deltaSum=0.0;
    $today=date('Y-m-d');foreach($interventions as $row){$status=(string)$row['status'];if(isset($intSummary[$status]))$intSummary[$status]++;if(in_array($status,['Pendiente','En proceso'],true)&&!empty($row['followup_date'])&&$row['followup_date']<$today)$intSummary['Seguimientos vencidos']++;
        if($status==='Completada'&&$row['risk_probability']!==null&&$row['post_risk_probability']!==null){$delta=(float)$row['risk_probability']-(float)$row['post_risk_probability'];$deltaSum+=$delta;$measured++;if($delta>0)$improved++;}}
    $metrics=(array)($model['metrics']['holdout']??[]);
    return ['ok'=>true,'model'=>['available'=>!empty($model['available']),'reason'=>$model['reason']??null,'created_at'=>$model['created_at']??null,'metrics'=>$metrics,'training'=>$model['training']??[]],
        'summary'=>['evaluated'=>$evaluated,'high'=>$summary['Alto'],'medium'=>$summary['Medio'],'low'=>$summary['Bajo'],'students_considered'=>count($students)],'predictions'=>$predictions,'factors'=>$factors,'interventions'=>$interventions,'intervention_summary'=>$intSummary,
...
function edu_risk_counterfactual_chat_result(mysqli $conn,array $actor,string $name,array $entities=[]): array {
    ...
    if(count($students)>1){$lines=[];foreach($students as $i=>$s)$lines[]=($i+1).'. '.$s['name'].' — '.$s['nivel'].' · '.$s['grado'].'° '.$s['seccion'];return edu_chat_result("Encontré varias coincidencias. Especifica el nombre completo o el aula:\n".implode("\n",$lines));}
    $cf=edu_risk_counterfactual_for_student($conn,$actor,(int)$students[0]['id'],!empty($entities['bimestre'])?(int)$entities['bimestre']:null);
    if(empty($cf['available']))return edu_chat_result('No hay datos suficientes para simular cambios de riesgo para ese estudiante.');
    $p=$cf['prediction'];$lines=[];$lines[]=$cf['student']['name'].' — riesgo actual '.$p['level'].' ('.number_format($p['probability']*100,1).'%).';
    ...
    else{$s=$cf['recommended'];$lines[]='Escenario simulado: '.$s['name'].'.';foreach($s['changes'] as $c){$before=$c['feature']==='attendance_rate_30d'?number_format($c['before'],1).'%':number_format($c['before'],1);$after=$c['feature']==='attendance_rate_30d'?number_format($c['after'],1).'%':number_format($c['after'],1);$lines[]='• '.$c['label'].': '.$before.' → '.$after.'.';}$lines[]='Riesgo estimado bajo ese escenario: '.$s['level'].' ('.number_format($s['probability']*100,1).'%).';}
...
function edu_risk_interventions_chat_result(mysqli $conn,array $actor,string $name='',array $entities=[]): array {
    if(!edu_risk_interventions_ready($conn)){ $r=edu_chat_result('El seguimiento de intervenciones aún no está habilitado en esta base de datos.');$r['tools_used']=['predictive_interventions'];return$r; }
    $filters=$entities;$student=null;
    if(trim($name)!==''){$matches=edu_predictive_students($conn,$actor,$entities,$name,8);if(!$matches)return edu_chat_result('No encontré un estudiante activo que coincida con ese nombre.');if(count($matches)>1){$lines=[];foreach($matches as $i=>$s)$lines[]=($i+1).'. '.$s['name'].' — '.$s['nivel'].' · '.$s['grado'].'° '.$s['seccion'];return edu_chat_result("Encontré varias coincidencias. Especifica el nombre completo o el aula:\n".implode("\n",$lines));}$student=$matches[0];$filters['student_id']=(int)$student['id'];}
    $rows=edu_risk_list_interventions($conn,$actor,$filters,30);if(!$rows){$msg=$student?'No hay intervenciones registradas para '.$student['name'].'.':'No hay intervenciones registradas con esos filtros.';$r=edu_chat_result($msg,[],[],[edu_chat_action('Abrir Alertas e Intervenciones','risk_dashboard','fa-brain')]);$r['tools_used']=['predictive_interventions'];return$r;}
    ...
    $r=edu_chat_result("Intervenciones registradas:\n".implode("\n",$lines),[],[['label'=>'Intervenciones','value'=>(string)count($rows),'tone'=>'primary']],[edu_chat_action('Abrir Alertas e Intervenciones','risk_dashboard','fa-brain')]);$r['tools_used']=['predictive_interventions'];return$r;
}

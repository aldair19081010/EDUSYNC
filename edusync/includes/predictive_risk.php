 ...
 *
 * Política temporal:
 * - Inferencia en vivo: usa el último bimestre institucionalmente cerrado y
 *   estima el riesgo en el bimestre inmediatamente siguiente, aunque este esté
 *   actualmente en proceso.
 * - Entrenamiento histórico: solo crea una etiqueta cuando tanto el bimestre
 *   base como el siguiente están cerrados, de modo que el resultado futuro sea
 *   completo y observable.
 * - La asistencia se corta en la fecha de cierre (o última fecha académica
 *   segura del bimestre cerrado), nunca en la fecha actual.
 *
 * No se usan datos financieros como predictores.
 */
...
function edu_predictive_model_load(): array {
    ...
    if ((int)($model['schema_version'] ?? 0) < 3) return ['available'=>false,'reason'=>'model_outdated','path'=>$path];
...
function edu_predictive_bimester_label(int $bimester): string {
    return [1=>'I',2=>'II',3=>'III',4=>'IV'][$bimester] ?? (string)$bimester;
}
...
function edu_predictive_grade_features(mysqli $conn, int $studentId, int $schoolId, int $yearId, int $bimester): array {
    ...
    foreach ($rows as $row) {
        ...
        if (edu_predictive_grade_is_critical($row['grade'] ?? null)) {
            $criticalRecords++;
            $key = (int)($row['course_id'] ?? 0) > 0 ? 'id:'.(int)$row['course_id'] : 'name:'.(string)($row['course_name'] ?? 'Sin curso');
            $criticalCourses[$key] = (string)($row['course_name'] ?? 'Sin curso');
        }
    }
    return [
...
function edu_predictive_eval_date_column(mysqli $conn): ?string {
    foreach (['date_created','created_at','evaluation_date','date'] as $column) {
        if (edu_predictive_column_exists($conn,'evaluations',$column)) return $column;
    }
    return null;
}

function edu_predictive_safe_eval_cutoff(mysqli $conn, int $schoolId, int $yearId, int $bimester): ?string {
    $column = edu_predictive_eval_date_column($conn);
    if ($column === null) return null;
    $where = ['tc.school_id=?','CAST(e.bimestre AS UNSIGNED)=?'];
    $types = 'ii';
    $params = [$schoolId,$bimester];
    if (edu_predictive_column_exists($conn,'teacher_courses','academic_year_id')) {
        $where[] = 'tc.academic_year_id=?';
        $types .= 'i';
        $params[] = $yearId;
    }
    $sql = "SELECT MAX(DATE(e.`$column`)) cutoff_date FROM evaluations e INNER JOIN teacher_courses tc ON tc.id=e.teacher_course_id WHERE ".implode(' AND ',$where);
    $stmt = $conn->prepare($sql);
    if (!$stmt) return null;
    edu_predictive_bind($stmt,$types,$params);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    return !empty($row['cutoff_date']) ? (string)$row['cutoff_date'] : null;
}

/**
 * Determina si un bimestre está realmente cerrado.
 * Prioridad de evidencia:
 * 1) bloqueo institucional (bimester_locks / academic_periods),
 * 2) todas las asignaciones docentes cerradas en grade_period_closures.
 *
 * Un bimestre con notas parciales pero sin cierre NO se considera cerrado.
 */
function edu_predictive_bimester_closure(mysqli $conn, int $schoolId, int $yearId, int $bimester): array {
    $out = ['closed'=>false,'date'=>null,'source'=>'none','institutional_lock'=>null,'closed_assignments'=>0,'total_assignments'=>0];
    if ($schoolId <= 0 || $yearId <= 0 || $bimester < 1 || $bimester > 4) return $out;

    $lockFound = false;
    if (edu_predictive_table_exists($conn,'bimester_locks')) {
        $stmt = $conn->prepare('SELECT is_locked FROM bimester_locks WHERE school_id=? AND academic_year_id=? AND bimester=? LIMIT 1');
        if ($stmt) {
            ...
            if ($row) {
                $lockFound = true;
                $out['institutional_lock'] = (int)$row['is_locked'] === 1;
                $out['closed'] = (bool)$out['institutional_lock'];
                $out['source'] = $out['closed'] ? 'bimester_locks' : 'bimester_locks_open';
            }
        }
    }

    if (!$lockFound && edu_predictive_table_exists($conn,'academic_periods')
        && edu_predictive_column_exists($conn,'academic_periods','is_locked')
        && edu_predictive_column_exists($conn,'academic_periods','period_number')) {
        $stmt = $conn->prepare('SELECT is_locked FROM academic_periods WHERE school_id=? AND academic_year_id=? AND period_number=? LIMIT 1');
        if ($stmt) {
            $stmt->bind_param('iii',$schoolId,$yearId,$bimester);
            $stmt->execute();
            $row = $stmt->get_result()->fetch_assoc();
            ...
            if ($row) {
                ...
                $out['institutional_lock'] = (int)$row['is_locked'] === 1;
                $out['closed'] = (bool)$out['institutional_lock'];
                $out['source'] = $out['closed'] ? 'academic_periods' : 'academic_periods_open';
            }
        }
    }

    if (edu_predictive_table_exists($conn,'teacher_courses') && edu_predictive_column_exists($conn,'teacher_courses','academic_year_id')) {
        $stmt = $conn->prepare('SELECT COUNT(*) total FROM teacher_courses WHERE school_id=? AND academic_year_id=?');
        if ($stmt) {
            $stmt->bind_param('ii',$schoolId,$yearId);
            $stmt->execute();
            $out['total_assignments'] = (int)($stmt->get_result()->fetch_assoc()['total'] ?? 0);
            $stmt->close();
        }
    }

    if (edu_predictive_table_exists($conn,'grade_period_closures')) {
        $hasYear = edu_predictive_column_exists($conn,'grade_period_closures','academic_year_id');
        $hasStatus = edu_predictive_column_exists($conn,'grade_period_closures','status');
        $hasClosedAt = edu_predictive_column_exists($conn,'grade_period_closures','closed_at');
        $where = ['school_id=?','bimester=?'];
        $types = 'ii';
        $params = [$schoolId,$bimester];
        if ($hasYear) { $where[]='academic_year_id=?'; $types.='i'; $params[]=$yearId; }
        if ($hasStatus) $where[]="LOWER(TRIM(status))='cerrado'";
        $dateSelect = $hasClosedAt ? ',MAX(DATE(closed_at)) cutoff_date' : ",NULL cutoff_date";
        $sql = 'SELECT COUNT(DISTINCT teacher_course_id) closed_total'.$dateSelect.' FROM grade_period_closures WHERE '.implode(' AND ',$where);
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            edu_predictive_bind($stmt,$types,$params);
            $stmt->execute();
            $row = $stmt->get_result()->fetch_assoc();
            $stmt->close();
            $out['closed_assignments'] = (int)($row['closed_total'] ?? 0);
            if (!empty($row['cutoff_date'])) $out['date'] = (string)$row['cutoff_date'];
        }
    }

    if (!$lockFound && $out['total_assignments'] > 0 && $out['closed_assignments'] >= $out['total_assignments']) {
        $out['closed'] = true;
        $out['source'] = 'all_grade_period_closures';
    }

    if ($out['closed'] && empty($out['date'])) {
        $out['date'] = edu_predictive_safe_eval_cutoff($conn,$schoolId,$yearId,$bimester);
        if ($out['date'] !== null) $out['source'] .= '+evaluation_cutoff';
    }
    return $out;
}

function edu_predictive_latest_closed_bimester(mysqli $conn, int $schoolId, int $yearId): ?int {
    for ($bimester = 3; $bimester >= 1; $bimester--) {
        $closure = edu_predictive_bimester_closure($conn,$schoolId,$yearId,$bimester);
        if (!empty($closure['closed'])) return $bimester;
    }
    return null;
}

/** Compatibilidad: ahora "latest" significa último bimestre cerrado, no último con alguna nota. */
function edu_predictive_latest_bimester(mysqli $conn, int $studentId, int $schoolId, int $yearId): ?int {
    return edu_predictive_latest_closed_bimester($conn,$schoolId,$yearId);
}
...
/** Solo produce etiqueta histórica cuando base y resultado futuro están cerrados. */
function edu_predictive_target_next_bimester(mysqli $conn,int $studentId,int $schoolId,int $yearId,int $bimester): ?int {
    ...
    if(empty($baseClosure['closed'])||empty($targetClosure['closed']))return null;
...
function edu_predictive_sigmoid(float $z): float {
    ...
    $ez=exp(max($z,-700.0));return $ez/(1.0+$ez);
}

function edu_predictive_score(array $features,array $model): array {
    $z=(float)$model['intercept'];$contributions=[];$imputer=(array)($model['imputer']['fill']??[]);
    foreach((array)$model['features'] as $feature){
        $raw=$features[$feature]??null;$missing=$raw===null||$raw===''||!is_numeric($raw);
        ...
        $standardized=($x-$mean)/$scale;$coef=(float)($model['coefficients'][$feature]??0.0);$contribution=$coef*$standardized;
        $z+=$contribution;$contributions[$feature]=['raw'=>$x,'missing'=>$missing,'standardized'=>$standardized,'coefficient'=>$coef,'contribution'=>$contribution];
    }
    $probability=edu_predictive_sigmoid($z);$medium=(float)($model['risk_thresholds']['medium']??0.40);$high=(float)($model['risk_thresholds']['high']??0.70);
...
function edu_predictive_feature_label(string $feature): string {
    $labels=['grade_mean_current'=>'promedio académico actual','grade_mean_previous'=>'promedio académico previo','grade_trend'=>'tendencia del rendimiento','critical_records_current'=>'cantidad de registros críticos actuales','critical_courses_current'=>'cantidad de cursos con registros críticos','attendance_rate_30d'=>'porcentaje de asistencia de los últimos 30 días','late_30d'=>'tardanzas de los últimos 30 días','absent_30d'=>'ausencias de los últimos 30 días'];
...
function edu_predictive_explanation(array $score,int $maxFactors=4): array {
    ...
    foreach($score['contributions'] as $feature=>$info){
        if(!empty($info['missing']))continue;$value=(float)$info['contribution'];if(abs($value)<0.05)continue;
...
function edu_predictive_student_prediction(mysqli $conn,array $actor,int $studentId,?int $bimester=null): array {
    $schoolId=(int)($actor['school_id']??0);$year=edu_predictive_academic_year($conn,$schoolId,null);
    if(!$year)return ['available'=>false,'reason'=>'academic_year'];
    $yearId=(int)$year['id'];
    $bimester=$bimester?:edu_predictive_latest_closed_bimester($conn,$schoolId,$yearId);
    if(!$bimester)return ['available'=>false,'reason'=>'no_closed_bimester'];
    if($bimester>=4)return ['available'=>false,'reason'=>'no_next_bimester','bimester'=>$bimester];

    $closure=edu_predictive_bimester_closure($conn,$schoolId,$yearId,$bimester);
    if(empty($closure['closed']))return ['available'=>false,'reason'=>'bimester_not_closed','bimester'=>$bimester];
    if(empty($closure['date']))return ['available'=>false,'reason'=>'closure_cutoff_missing','bimester'=>$bimester];

    $model=edu_predictive_model_load();if(empty($model['available']))return $model;
    $window=(int)($model['training']['attendance_window_days']??30);
    $features=edu_predictive_feature_vector($conn,$studentId,$schoolId,$yearId,$bimester,(string)$closure['date'],$window);
    ...
    return [
        'available'=>true,
        'bimester'=>$bimester,
        'bimester_label'=>edu_predictive_bimester_label($bimester),
        'target_bimester'=>$bimester+1,
        'target_bimester_label'=>edu_predictive_bimester_label($bimester+1),
        'base_closed_at'=>$closure['date'],
        'closure_source'=>$closure['source'],
        'target_closed'=>!empty($targetClosure['closed']),
        'features'=>$features,
        'data_quality'=>['attendance_available'=>(int)$features['_attendance_records']>0,'attendance_records'=>(int)$features['_attendance_records']],
        'probability'=>$score['probability'],'level'=>$score['level'],'explanation'=>edu_predictive_explanation($score),'model'=>$model
    ];
}
...
function edu_predictive_model_unavailable_message(array $model): string {
    ...
    if($reason==='model_missing')return 'El módulo predictivo ya está instalado, pero todavía no existe un modelo entrenado. Primero exporta el dataset histórico y entrena el modelo.';
    if($reason==='model_outdated')return 'El modelo predictivo fue generado con una política temporal anterior. Vuelve a exportar el dataset y reentrena para usar únicamente bimestres cerrados como base y como resultado histórico.';
...
function edu_predictive_chat_result(mysqli $conn,array $actor,array $entities=[],string $nameSearch='',int $limit=30): array {
    ...
    if(empty($model['available'])){$message=edu_predictive_model_unavailable_message($model);$result=function_exists('edu_chat_result')?edu_chat_result($message):['message'=>$message,'cards'=>[],'actions'=>[],'follow_up'=>[]];$result['tools_used']=['predictive_risk'];return$result;}
    ...
    $predictions=[];$requestedBimester=!empty($entities['bimestre'])?(int)$entities['bimestre']:null;
    foreach($students as $student){$p=edu_predictive_student_prediction($conn,$actor,(int)$student['id'],$requestedBimester);if(empty($p['available']))continue;$predictions[]=['student'=>$student,'prediction'=>$p];}
    if(!$predictions){$result=edu_chat_result($requestedBimester?'El bimestre solicitado no está cerrado o no hay datos suficientes para calcular la alerta.':'No existe todavía un bimestre cerrado con datos suficientes para calcular riesgo predictivo.');$result['tools_used']=['predictive_risk'];return$result;}
    usort($predictions,static fn($a,$b)=>$b['prediction']['probability']<=>$a['prediction']['probability']);$predictions=array_slice($predictions,0,max(1,min(50,$limit)));

    $first=$predictions[0]['prediction'];$baseLabel=$first['bimester_label'];$targetLabel=$first['target_bimester_label'];$cutoff=$first['base_closed_at'];
    ...
    $message="Riesgo académico estimado en el {$targetLabel} bimestre, calculado con información disponible al cierre del {$baseLabel} bimestre ({$cutoff}):\n".implode("\n",$lines);
    $message.="\nLa probabilidad es una alerta de apoyo y no una decisión automática.";
    if($missingAttendance>0)$message.="\nObservación: {$missingAttendance} estudiante(s) no tienen registros suficientes de asistencia en la ventana previa al cierre; esos valores se tratan como faltantes.";
    ...
    $result=edu_chat_result($message,['¿Por qué está en riesgo el primero?','Muéstrame solo secundaria','Dame los de riesgo alto'],$cards,$actions);$result['tools_used']=['predictive_risk'];return$result;
}

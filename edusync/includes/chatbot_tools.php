...
function edu_chat_ai_tool_definitions(array $actor): array {
    ...
    if (in_array($type,[1,2,3],true)) {
        $tools[] = edu_chat_ai_function(
            ...
            [
                'subject'=>['type'=>'string','enum'=>['students','teachers','courses','payments','debts','attendance','grades','evaluations','risk'],'description'=>'Dominio principal de la consulta. Respeta el perfil: docentes y auxiliares tienen menos dominios autorizados.'],
                'operation'=>['type'=>'string','enum'=>['summary','count','list','sum','average','distribution','ranking'],'description'=>'Operación solicitada.'],
                'group_by'=>['type'=>'string','enum'=>['none','level','grade','section','grade_section','course','bimestre','payment_method'],'description'=>'Agrupación cuando la operación es distribution.'],
                'level'=>$level,
                'grade'=>$grade,
                'section'=>$section,

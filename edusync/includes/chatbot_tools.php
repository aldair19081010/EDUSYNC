...
require_once __DIR__ . '/chatbot_semantic_schema.php';
...
function edu_chat_ai_tool_definitions(array $actor): array {
    ...
    $period = ['type'=>'string','enum'=>['today','yesterday','week','month','last_month','year'],'description'=>'Periodo solicitado. today=hoy, yesterday=ayer, week=esta semana, month=este mes, last_month=mes pasado, year=año académico actual.'];
    $paymentMethod = ['type'=>'string','enum'=>['Efectivo','Yape','Transferencia'],'description'=>'Método de pago canónico. La IA debe normalizar sinónimos como cash→Efectivo y depósito/banco→Transferencia.'];
    ...
    if ($type !== 4) {
        ...
        $universalFilters = [
            'type'=>'object',
            'description'=>'Filtros opcionales. Incluye solo los que el usuario haya pedido o sean necesarios por el contexto.',
            'properties'=>[
                'level'=>['type'=>'string','enum'=>['Inicial','Primaria','Secundaria']],
                'grade'=>['type'=>'string','description'=>'Grado como 1, 2, 3, 4, 5 o 6.'],
                'section'=>['type'=>'string','description'=>'Sección como A, B o U. Si pide ambas/todas las secciones, omitir este filtro.'],
                'name'=>['type'=>'string','description'=>'Nombre o parte del nombre de estudiante/docente/cliente según el dominio.'],
                'specialty'=>['type'=>'string','description'=>'Especialidad del docente.'],
                'course'=>['type'=>'string','description'=>'Curso/área académica.'],
                'teacher'=>['type'=>'string','description'=>'Nombre o parte del nombre del docente.'],
                'bimestre'=>['type'=>'string','enum'=>['1','2','3','4']],
                'year'=>['type'=>'string','description'=>'Año académico solicitado, por ejemplo 2026.'],
                'period'=>['type'=>'string','enum'=>['today','yesterday','week','month','last_month','year']],
                'date_from'=>['type'=>'string','description'=>'Fecha inicial YYYY-MM-DD cuando el usuario indique un rango exacto.'],
                'date_to'=>['type'=>'string','description'=>'Fecha final YYYY-MM-DD cuando el usuario indique un rango exacto.'],
                ...
                'attendance_type'=>['type'=>'string','enum'=>['Entrada','Salida'],'description'=>'Tipo de marcación. Si no se especifica, el motor usa Entrada.'],
                'grade_value'=>['type'=>'string','description'=>'Nota literal o exacta solicitada, por ejemplo C, B, A, AD o 15.'],
                'critical_only'=>['type'=>'boolean','description'=>'true cuando se solicitan únicamente registros académicos críticos (C o nota numérica menor a 10.5).'],
                'competency'=>['type'=>'string','description'=>'Nombre o parte del nombre de la competencia.'],
                'evaluation_type'=>['type'=>'string','description'=>'Tipo de evaluación, por ejemplo Examen, Tarea, Proyecto o Trabajo en clase.'],
                ...
                'concept'=>['type'=>'string','description'=>'Nombre o parte del concepto de pago. Úsalo con domain=concepts o domain=debts; no lo mezcles con método de pago si el pago fue dividido entre varios medios.'],
                ...
                'late_min'=>['type'=>'number'],'late_max'=>['type'=>'number'],
                'absent_min'=>['type'=>'number'],'absent_max'=>['type'=>'number'],
                'critical_min'=>['type'=>'number'],'critical_max'=>['type'=>'number'],
                'average_grade_min'=>['type'=>'number'],'average_grade_max'=>['type'=>'number'],
                'grade_min'=>['type'=>'number'],'grade_max'=>['type'=>'number']
            ]
        ];

        $tools[] = edu_chat_ai_function(
            'query_edusync_data',
            'Motor universal SEGURO de solo lectura. Úsalo cuando una pregunta sobre datos reales de EduSync no encaje exactamente en otra herramienta, cuando combine varios dominios/filtros, o cuando pida comparar, agrupar, ordenar o calcular. Nunca envía SQL: solo un plan semántico validado por PHP. Dominio students permite cruces como deuda + pagos + tardanzas/ausencias + riesgo/promedio. Usa herramientas específicas cuando respondan exactamente y este motor para lo demás.',
            [
    ...
    if ($type === 1) {
        ...
        $tools[] = edu_chat_ai_function('get_collections_summary', 'Resume cobranza confirmada del colegio autenticado sin filtrar por método de pago.', ['period'=>$period,'level'=>$level]);
        $tools[] = edu_chat_ai_function('get_collections_by_method', 'Consulta cobranza confirmada filtrada por método de pago. Úsala para preguntas sobre efectivo, Yape o transferencia, incluso si el usuario dice entró, ingresó, recibimos, cobramos o caja.', ['payment_method'=>$paymentMethod,'period'=>$period,'level'=>$level,'grade'=>$grade,'section'=>$section], ['payment_method']);
...
function edu_chat_ai_entities(array $args): array {
    return [
        ...
        'payment_method'=>isset($args['payment_method']) && $args['payment_method']!=='' ? (string)$args['payment_method'] : null
    ];
}
...
function edu_chat_ai_run_tool(mysqli $conn, array $actor, string $name, array $args): array {
    ...
    switch($name){
        ...
        case 'get_collections_by_method': if($type!==1)break; if(empty($entities['period']))$entities['period']='month'; $method=(string)($args['payment_method']??''); if(!in_array($method,['Efectivo','Yape','Transferencia'],true))return edu_chat_result('No reconocí un método de pago válido.'); return edu_chat_adv_collection_method_result($conn,$actor,$entities,$method);

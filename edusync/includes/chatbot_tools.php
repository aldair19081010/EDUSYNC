...
function edu_chat_ai_tool_definitions(array $actor): array {
    ...
    if ($type === 1) {
        ...
        $tools[] = edu_chat_ai_function('get_student_count', 'Devuelve UN SOLO TOTAL de estudiantes activos con filtros opcionales. NO la uses para preguntas con cada, por grado, por sección, distribución, desglose o aulas; para esas preguntas usa get_student_distribution.', ['level'=>$level,'grade'=>$grade,'section'=>$section]);
        $tools[] = edu_chat_ai_function('get_student_distribution', 'Devuelve conteos REALES agrupados de estudiantes activos. Úsala SIEMPRE para preguntas como cuántos hay en cada sección, por grado, por nivel, distribución de alumnos o cantidad por aula.', ['group_by'=>$groupBy,'level'=>$level,'grade'=>$grade,'section'=>$section]);
        $tools[] = edu_chat_ai_function('get_student_roster', 'Lista nombres de estudiantes activos con nivel, grado y sección. Úsala para preguntas como quiénes son, lista los alumnos, busca a un estudiante o alumnos de un aula.', ['name_search'=>$nameSearch,'limit'=>$limit50,'level'=>$level,'grade'=>$grade,'section'=>$section]);
        ...
        $tools[] = edu_chat_ai_function('get_academic_risk_distribution', 'Desglosa estudiantes en riesgo académico por nivel, grado, sección, aula o curso usando datos reales del año actual.', ['group_by'=>$riskGroupBy,'level'=>$level,'grade'=>$grade,'section'=>$section,'bimestre'=>$bimestre,'course'=>$course]);
        return $tools;
    }
    ...
    if ($type === 2) {
        ...
        $tools[] = edu_chat_ai_function('get_my_student_count', 'Devuelve un total de estudiantes vinculados a las asignaciones del docente. Para desglose por aula usa get_student_distribution.', ['level'=>$level,'grade'=>$grade]);
        $tools[] = edu_chat_ai_function('get_student_distribution', 'Desglosa únicamente los estudiantes vinculados al docente por nivel, grado o sección.', ['group_by'=>$groupBy,'level'=>$level,'grade'=>$grade,'section'=>$section]);
    ...
    if ($type === 3) {
        ...
        $tools[] = edu_chat_ai_function('get_student_count', 'Devuelve un total de estudiantes activos. Para conteos por aula/sección usa get_student_distribution.', ['level'=>$level,'grade'=>$grade,'section'=>$section]);
        $tools[] = edu_chat_ai_function('get_student_distribution', 'Desglosa estudiantes activos por nivel, grado o sección. Úsala para cada sección o por aula.', ['group_by'=>$groupBy,'level'=>$level,'grade'=>$grade,'section'=>$section]);

(function($){
    ...
    function context(){
        return {assignment: String($('#gb-assignment').val()||''), bimester: String($('#gb-bimester').val()||'')};
    }
    ...
    function applyReadOnlyHint(closed){
        if(closed){
            $('#gb-readonly-banner').show().find('span').text('Las notas de este curso y bimestre fueron cerradas por el docente.');
        ...
        } else if(!($('#gb-readonly-banner').is(':visible') && /bloqueado|histórico|cerrado/i.test($('#gb-readonly-banner').text()))){
    ...
    function render(data){
        ...
        var sub=closed?'Las notas están protegidas y son de solo lectura.':(data.ready?'Todas las validaciones están correctas.':'Corrige los pendientes antes de cerrar el bimestre.');
        ...
        panel.html('<div class="d-flex gc-main justify-content-between align-items-start"><div class="pr-md-3 flex-grow-1"><div class="'+color+' gc-state"><i class="fas '+icon+' mr-1"></i>'+esc(state)+'</div><div class="font-weight-bold text-gray-800">'+title+'</div><div class="small text-muted">'+sub+'</div>'+meta+'<div class="mt-2">'+summaryHtml(s)+'</div><div class="d-flex justify-content-between small mt-2"><span>Completitud de calificaciones</span><strong>'+progress.toFixed(1)+'%</strong></div><div class="progress gc-progress"><div class="progress-bar '+(progress>=100?'bg-success':'bg-warning')+'" style="width:'+Math.min(100,progress)+'%"></div></div></div><div class="gc-actions d-flex">'+actions+'</div></div><div id="gc-detail-area" style="display:none">'+detailsHtml(validation)+'</div>');
        applyReadOnlyHint(closed);
    }
    function renderError(message,migration){
        panel.removeClass('gc-ready gc-closed').addClass('gc-incomplete').html('<div class="alert '+(migration?'alert-warning':'alert-danger')+' mb-0"><i class="fas '+(migration?'fa-database':'fa-exclamation-circle')+' mr-1"></i>'+esc(message)+'</div>');
    }
    function refresh(force){
        if(!validContext()){
            currentKey=contextKey();currentData=null;
            panel.removeClass('gc-ready gc-closed gc-incomplete').html('<div class="text-muted small"><i class="fas fa-lock mr-1"></i>Selecciona un curso y un bimestre para verificar el cierre.</div>');
            return;
        }
        var key=contextKey();
        if(loading&&!force)return;
        loading=true;currentKey=key;
        $.getJSON(api,{action:'status',teacher_course_id:context().assignment,bimestre:context().bimester})
            .done(function(r){if(contextKey()!==key)return;if(!r||Number(r.status)!==1){renderError((r&&r.message)||'No se pudo verificar el cierre.',!!(r&&r.migration_required));return;}lastRefresh=Date.now();render(r);})
            .fail(function(x){if(contextKey()!==key)return;var r=x.responseJSON||{};renderError(r.message||'No se pudo verificar el estado del bimestre.',!!r.migration_required);})
    ...
    $(document).on('click','#gc-confirm-close',function(){
        ...
        $.post(api+'?action=close',{csrf_token:csrf,teacher_course_id:context().assignment,bimestre:context().bimester},null,'json')
            .done(function(r){if(!r||Number(r.status)!==1){if(typeof alert_toast==='function')alert_toast((r&&r.message)||'No se pudo cerrar el bimestre.','danger');if(r&&r.validation)refresh(true);return;}$('#gc-close-modal').modal('hide');if(typeof alert_toast==='function')alert_toast(r.message,'success');reloadBook();})
            .fail(function(x){if(typeof alert_toast==='function')alert_toast((x.responseJSON||{}).message||'No se pudo cerrar el bimestre.','danger');})
    ...
    $(document).on('click','#gc-send-reopen',function(){
        var reason=$.trim($('#gc-reopen-reason').val());if(reason.length<5){if(typeof alert_toast==='function')alert_toast('Explica el motivo de la reapertura.','warning');return;}
        var btn=$(this).prop('disabled',true).html('<i class="fas fa-spinner fa-spin mr-1"></i>Enviando...');
        $.post(api+'?action=request_reopen',{csrf_token:csrf,teacher_course_id:context().assignment,bimestre:context().bimester,reason:reason},null,'json')
            .done(function(r){if(!r||Number(r.status)!==1){if(typeof alert_toast==='function')alert_toast((r&&r.message)||'No se pudo enviar la solicitud.','danger');return;}$('#gc-reopen-modal').modal('hide');if(typeof alert_toast==='function')alert_toast(r.message,'success');refresh(true);if(typeof window.refreshUnifiedNotifications==='function')window.refreshUnifiedNotifications();})
            .fail(function(x){if(typeof alert_toast==='function')alert_toast((x.responseJSON||{}).message||'No se pudo enviar la solicitud.','danger');})
            .always(function(){btn.prop('disabled',false).html('<i class="fas fa-paper-plane mr-1"></i>Enviar solicitud');});
    });

    $('#gb-year,#gb-course,#gb-level,#gb-grade,#gb-section,#gb-bimester').on('change.gradeClosure',function(){setTimeout(function(){refresh(true);},120);});
    $('#gb-load').on('click.gradeClosure',function(){setTimeout(function(){refresh(true);},150);});
    $(document).on('evaluation:saved.gradeClosure evaluation:gradesSaved.gradeClosure',function(){setTimeout(function(){refresh(true);},300);});
    $(document).on('grade:reopen-reviewed.gradeClosure',function(){refresh(true);reloadBook();});

    window.setInterval(function(){
        var key=contextKey();
        if(key!==currentKey){refresh(true);return;}
        if(validContext()&&Date.now()-lastRefresh>15000&&!loading)refresh(false);
    },1000);
})(window.jQuery);

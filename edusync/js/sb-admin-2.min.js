...
!function(l){"use strict";l("#sidebarToggle, #sidebarToggleTop").on("click",function(e){l("body").toggleClass("sidebar-toggled"),l(".sidebar").toggleClass("toggled"),l(".sidebar").hasClass("toggled")&&l(".sidebar .collapse").collapse("hide")}),l(window).resize(function(){l(window).width()<768&&l(".sidebar .collapse").collapse("hide"),l(window).width()<480&&!l(".sidebar").hasClass("toggled")&&(l("body").addClass("sidebar-toggled"),l(".sidebar").addClass("toggled"),l(".sidebar .collapse").collapse("hide"))}),l("body.fixed-nav .sidebar").on("mousewheel DOMMouseScroll wheel",function(e){var o;768<l(window).width()&&(o=(o=e.originalEvent).wheelDelta||-o.detail,this.scrollTop+=30*(o<0?1:-1),e.preventDefault())}),l(document).on("scroll",function(){100<l(this).scrollTop()?l(".scroll-to-top").fadeIn():l(".scroll-to-top").fadeOut()}),l(document).on("click","a.scroll-to-top",function(e){var o=l(this),href=o.attr("href");if(href&&href!=="#"&&href.length>1)try{var t=l(href);t.length&&l("html, body").stop().animate({scrollTop:t.offset().top},1e3,"easeInOutExpo")}catch(n){}e.preventDefault()})}(jQuery);
...
(function($){
  ...
  function roundGrade(value){
    return value===null||value===undefined||isNaN(value)?null:Math.round(Number(value));
  }
  ...
  function badge(status,extra){
    return $('<span>',{class:'ir-status-badge '+status.className+(extra?' '+extra:''),text:status.letter+' · '+status.label});
  }
    ...
    $report.find('.ir-card table').each(function(){
      ...
      $table.find('tbody tr').each(function(){
        $(this).find('td,th').each(function(i){
          if(headers[i]!=='promedio')return;
          var value=parseGrade($(this).text());
          if(value!==null)$(this).text(roundGrade(value));
        });
      });
  ...
  function buildSummary($report,states){
    ...
    var $grid=$('<div>',{class:'ir-summary-grid','aria-label':'Resumen del desempeño'});
  ...
  function trendRequest(){
    return {
      report_view:'comparison',data_mode:'trend',
      academic_year_id:$('#academic_year_id').val()||'',
      course_id:$('#course_id').val()||'',student_id:$('#student_id').val()||'',
      bimestre:$('#bimestre').val()||'',level:$('#level').val()||'',
      grado:$('#grado').val()||'',seccion:$('#seccion').val()||''
    };
  }
  function scoreArray(obj){
    var arr=[];for(var i=1;i<=4;i++){var v=obj?obj[i]:null;arr.push(v===null||v===undefined?null:roundGrade(v));}return arr;
  }
  function annualAverage(scores){
    var valid=scores.filter(function(v){return v!==null;});
    return valid.length?roundGrade(valid.reduce(function(a,b){return a+b;},0)/valid.length):null;
  }
  function buildDualSvg(studentScores,classScores,selected){
    var width=780,height=300,left=58,right=28,top=42,bottom=48,plotW=width-left-right,plotH=height-top-bottom;
    var xs=[0,1,2,3].map(function(i){return left+(plotW/3)*i;});
    function y(v){return top+plotH-(Math.max(0,Math.min(20,v))/20)*plotH;}
    var out=[];
    out.push('<svg viewBox="0 0 '+width+' '+height+'" role="img" aria-label="Comparación bimestral del alumno con el promedio del aula">');
    out.push('<rect x="0" y="0" width="'+width+'" height="'+height+'" rx="8" fill="#fff"/>');
    out.push('<line x1="'+left+'" y1="18" x2="'+(left+24)+'" y2="18" stroke="#4e73df" stroke-width="4"/><text x="'+(left+32)+'" y="22" font-size="11" fill="#596579">Alumno</text>');
    out.push('<line x1="'+(left+105)+'" y1="18" x2="'+(left+129)+'" y2="18" stroke="#858796" stroke-width="3" stroke-dasharray="7 5"/><text x="'+(left+137)+'" y="22" font-size="11" fill="#596579">Promedio del aula</text>');
    ...
    function drawLine(scores,color,dashed){
      for(var i=0;i<3;i++){if(scores[i]===null||scores[i+1]===null)continue;out.push('<line x1="'+xs[i]+'" y1="'+y(scores[i])+'" x2="'+xs[i+1]+'" y2="'+y(scores[i+1])+'" stroke="'+color+'" stroke-width="'+(dashed?3:4)+'" '+(dashed?'stroke-dasharray="7 5"':'')+' stroke-linecap="round"/>');}
    }
  ...
  function trendText(studentScores,classScores){
    var valid=[];studentScores.forEach(function(v,i){if(v!==null)valid.push({bi:i+1,v:v});});
    var parts=[];
  ...
  function renderTrend($report,data){
    var studentScores=scoreArray(data.student_scores),classScores=scoreArray(data.class_scores),selected=parseInt($('#bimestre').val(),10)||0;
    var studentAnnual=annualAverage(studentScores),classAnnual=annualAverage(classScores),best=null,bestBi=null;
    studentScores.forEach(function(v,i){if(v!==null&&(best===null||v>best)){best=v;bestBi=i+1;}});
    var current=selected?studentScores[selected-1]:null,previous=selected>1?studentScores[selected-2]:null,variation=(current!==null&&previous!==null)?current-previous:null,classCurrent=selected?classScores[selected-1]:null,diff=(current!==null&&classCurrent!==null)?current-classCurrent:null;
    var $card=$('<div>',{class:'ir-trend-card'}),$head=$('<div>',{class:'ir-trend-head'}),$body=$('<div>',{class:'ir-trend-body'});
    ...
    var $metrics=$('<div>',{class:'ir-trend-metrics'});metrics.forEach(function(m){var $box=$('<div>',{class:'ir-trend-metric'});$box.append($('<small>',{text:m.label}));$box.append($('<strong>',{text:m.value,class:m.cls||''}));$metrics.append($box);});
  ...
  function viewName(){var v=$('#report_view').val()||'detail',names={detail:'detalle',consolidated:'consolidado',student:'ficha_individual',pending:'seguimiento',comparison:'comparacion_bimestral'};return names[v]||v;}
  function filtersText(){return [$('#academic_year_id option:selected').text(),$('#level').val(),$('#grado').val(),$('#seccion').val(),$('#course_id option:selected').text(),$('#bimestre option:selected').text()].filter(function(v){return v&&String(v).trim()&&String(v).trim()!=='Todos'&&String(v).trim()!=='Seleccionar';}).join(' · ');}
  function tableRows($table){
    var rows=[];
    if($.fn.dataTable&&$.fn.dataTable.isDataTable($table[0])){$($table.DataTable().rows({search:'applied'}).nodes()).each(function(){rows.push($(this).find('td').map(function(){return $(this).text().trim();}).get());});}
    else $table.find('tbody tr').each(function(){rows.push($(this).find('td').map(function(){return $(this).text().trim();}).get());});
    return rows;
  }
  function tableHtml($table){
    var $clone=$table.clone();$clone.removeClass('dataTable').removeAttr('style').css('width','100%');var rows=tableRows($table),$body=$clone.find('tbody').empty();rows.forEach(function(row){var $tr=$('<tr>');row.forEach(function(cell){$tr.append($('<td>').text(cell));});$body.append($tr);});return $('<div>').append($clone).html();
  }
  function exportCurrentExcel(e){
    if(e)e.preventDefault();var $tables=$('#grades-report-table table');if(!$tables.length){if(typeof alert_toast==='function')alert_toast('No hay datos para exportar.','warning');return;}
    var html='<!doctype html><html><head><meta charset="utf-8"><style>body{font-family:Arial,sans-serif}table{border-collapse:collapse;margin-bottom:18px;width:100%}th,td{border:1px solid #999;padding:5px}th{background:#eef2f8}</style></head><body><h2>Reporte de notas - '+viewName().replace(/_/g,' ')+'</h2><p>'+ $('<div>').text(filtersText()).html() +'</p>';
    $tables.each(function(i){html+='<h3>Tabla '+(i+1)+'</h3>'+tableHtml($(this));});html+='</body></html>';
    var blob=new Blob(['\ufeff',html],{type:'application/vnd.ms-excel;charset=utf-8;'}),link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='reporte_notas_'+viewName()+'.xls';document.body.appendChild(link);link.click();document.body.removeChild(link);URL.revokeObjectURL(link.href);
  }
  function exportCurrentCsv(e){
    if(e)e.preventDefault();var $tables=$('#grades-report-table table');if(!$tables.length){if(typeof alert_toast==='function')alert_toast('No hay datos para exportar.','warning');return;}
    var rows=[],quote=function(v){return '"'+String(v==null?'':v).replace(/"/g,'""')+'"';};
    rows.push(['Reporte de notas - '+viewName().replace(/_/g,' ')]);rows.push([filtersText()]);
    $tables.each(function(i){var $table=$(this);rows.push([]);rows.push(['Tabla '+(i+1)]);rows.push($table.find('thead tr').last().find('th').map(function(){return $(this).text().trim();}).get());tableRows($table).forEach(function(r){rows.push(r);});});
    var csv='\ufeff'+rows.map(function(r){return r.map(quote).join(';');}).join('\r\n'),blob=new Blob([csv],{type:'text/csv;charset=utf-8;'}),link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='reporte_notas_'+viewName()+'.csv';document.body.appendChild(link);link.click();document.body.removeChild(link);URL.revokeObjectURL(link.href);
  }
  function syncExportMenu(){
    if(!$('#export-dropdown').length)return;
    if(!$('#export-view-excel').length){var $item=$('<a>',{href:'#',id:'export-view-excel',class:'dropdown-item'}).html('<i class="fa fa-table text-success"></i> Excel de esta vista');$('#export-csv-report').before($item);}
    $('#export-excel-report').toggle(($('#report_view').val()||'detail')==='detail');
  }
  function bindExports(){
    if(!$('#filter-form').length)return;syncExportMenu();$('#export-view-excel').off('click').on('click',exportCurrentExcel);$('#export-csv-report').off('click').on('click',exportCurrentCsv);$(document).off('click.grExportView','.report-view').on('click.grExportView','.report-view',function(){setTimeout(syncExportMenu,0);});
  }

  $(function(){scanReports();bindExports();});
  if(window.MutationObserver){new MutationObserver(function(mutations){var scan=false;mutations.forEach(function(m){if(m.addedNodes&&m.addedNodes.length)scan=true;});if(scan)scanReports();}).observe(document.documentElement,{childList:true,subtree:true});}
})(jQuery);

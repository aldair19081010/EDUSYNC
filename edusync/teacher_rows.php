...
<tr>
    ...
    <td class="text-center ed-actions-cell">
        <div class="ed-row-actions teacher-actions-menu">
            <button class="btn btn-outline-primary btn-sm ed-action-primary view_teacher_history" type="button" data-id="<?php echo (int)$row['id']; ?>" title="Ver ficha del docente">
                <i class="fa fa-eye"></i><span class="ed-action-label">Ver</span>
            </button>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right ed-action-menu">
                    <button class="dropdown-item edit_teacher" type="button" data-id="<?php echo (int)$row['id']; ?>"><i class="fa fa-edit text-primary"></i>Editar información</button>
                    <button class="dropdown-item create_user_teacher" type="button" data-id="<?php echo (int)$row['id']; ?>" data-name="<?php echo htmlspecialchars($row['name'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" data-email="<?php echo htmlspecialchars($row['email'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" title="<?php echo $userCount > 0 ? 'Gestionar usuario' : 'Crear usuario'; ?>"><i class="fa fa-user-cog text-success"></i><?php echo $userCount > 0 ? 'Gestionar usuario' : 'Crear usuario'; ?></button>
                    <button class="dropdown-item manage_teacher_courses" type="button" data-id="<?php echo (int)$row['id']; ?>"><i class="fa fa-book text-warning"></i>Gestionar cursos</button>
                    <div class="dropdown-divider"></div>
                    <button class="dropdown-item delete_teacher <?php echo $teacherStatus === 'Activo' ? 'ed-action-danger' : 'ed-action-success'; ?>" type="button" data-id="<?php echo (int)$row['id']; ?>" data-status="<?php echo htmlspecialchars($teacherStatus, ENT_QUOTES, 'UTF-8'); ?>"><i class="fa <?php echo $teacherStatus === 'Activo' ? 'fa-user-slash' : 'fa-user-check'; ?>"></i><?php echo $teacherStatus === 'Activo' ? 'Desactivar docente' : 'Recontratar docente'; ?></button>
                </div>
            </div>
        </div>
    </td>

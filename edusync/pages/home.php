...
/* IMPORTANTE: Asegurar que el topbar está siempre visible */
.topbar {
    position: relative;
    z-index: 100 !important;
}

.btn-group, .dropdown-menu, [role="tooltip"], .popover {
...
/* Asegurar que los canvas de Chart.js no bloqueen clicks en dropdowns */
canvas {
    pointer-events: auto;
    position: relative;
    z-index: 1;
...
/* Elevar dropdowns sobre los gráficos */
.dropdown-menu {
...
/* Asegurar que topbar y sus elementos siempre estén encima */
.topbar {
    position: relative;
...
/* Permitir clicks en todos los elementos del topbar */
.topbar * {
    pointer-events: auto;
}
</style>
...
<script>
// Log INMEDIATO - antes de que cargue jQuery
console.log('Home page loaded - Role:', <?php echo json_encode($_SESSION['login_type'] ?? 0); ?>);
</script>

<!-- SIN div envolvente, contenido directo en container-fluid de index.php -->
<div style="max-width: 1000px; margin: 30px auto 0 auto; background: linear-gradient(to bottom, #ffffff, #f9f9fc); border-radius: 15px; box-shadow: 0 5px 20px rgba(66, 133, 244, 0.08); padding: 40px 30px 35px 30px; text-align: left; border-top: 4px solid #4285f4;">
    ...
    $students_query = "SELECT COUNT(id) as total_students FROM student WHERE status = 'Activo' AND school_id = $school_id";
    ...
    $teachers_query = "SELECT COUNT(id) as total_teachers FROM teacher WHERE school_id = $school_id";
    ...
    $courses_query = "SELECT COUNT(c.id) as total_courses FROM courses c INNER JOIN academic_year ay ON ay.id = c.academic_year_id WHERE ay.school_id = $school_id";
    ...
    $current_month_payments_query = "
                SELECT SUM(p.amount) as total_monthly_payments 
                FROM payments p
                INNER JOIN student_ef_list sef ON p.ef_id = sef.id
                INNER JOIN student s ON s.id = sef.student_id
                WHERE s.school_id = $school_id
                    AND MONTH(p.date_created) = MONTH(CURDATE()) AND YEAR(p.date_created) = YEAR(CURDATE())
    ";
    ...
    // Payments Today
    $payments_today_query = "SELECT SUM(p.amount) as total_payments_today FROM payments p INNER JOIN student_ef_list sef ON p.ef_id = sef.id INNER JOIN student s ON s.id = sef.student_id WHERE s.school_id = $school_id AND DATE(p.date_created) = CURDATE()";
    ...
    $payments_week_query = "SELECT SUM(p.amount) as total_payments_week FROM payments p INNER JOIN student_ef_list sef ON p.ef_id = sef.id INNER JOIN student s ON s.id = sef.student_id WHERE s.school_id = $school_id AND YEARWEEK(p.date_created, 1) = YEARWEEK(CURDATE(), 1)";
    ...
    // Payments This Year
    $payments_year_query = "SELECT SUM(p.amount) as total_payments_year FROM payments p INNER JOIN student_ef_list sef ON p.ef_id = sef.id INNER JOIN student s ON s.id = sef.student_id WHERE s.school_id = $school_id AND YEAR(p.date_created) = YEAR(CURDATE())";
    ...
    // Pagos del mes anterior (para tendencia)
        $prev_month_payments_query = "SELECT SUM(p.amount) as total_prev_month 
                FROM payments p
                INNER JOIN student_ef_list sef ON p.ef_id = sef.id
                INNER JOIN student s ON s.id = sef.student_id
                WHERE s.school_id = $school_id
                    AND YEAR(p.date_created) = YEAR(DATE_SUB(CURDATE(), INTERVAL 1 MONTH)) 
                    AND MONTH(p.date_created) = MONTH(DATE_SUB(CURDATE(), INTERVAL 1 MONTH))";
    ...
    $income_by_concept_query = "
        ...
        WHERE s.school_id = $school_id AND ay.school_id = $school_id
        GROUP BY c.course
    ...
    $monthly_income_trend_query = "
        SELECT 
            DATE_FORMAT(p.date_created, '%Y-%m') as month_year, 
            SUM(p.amount) as monthly_total
        FROM payments p
        INNER JOIN student_ef_list sef ON p.ef_id = sef.id
        INNER JOIN student s ON s.id = sef.student_id
        WHERE s.school_id = $school_id AND YEAR(p.date_created) = YEAR(CURDATE())
        GROUP BY month_year
    ...
    $expected_payments_month_query = "
        ...
        WHERE s.school_id = $school_id AND MONTH(sef.date_created) = MONTH(CURDATE()) AND YEAR(sef.date_created) = YEAR(CURDATE())
    ";
    ...
    $total_academic_courses_query = "SELECT COUNT(id) as total_academic_courses FROM academic_courses WHERE school_id = $school_id";
    ...
    $grade_scale_max = 0;
    $max_grade_sql = "SELECT MAX(CAST(eg.grade AS DECIMAL(6,2))) AS max_grade
    ...
    <!-- PARA DOCENTE, NO USAR CONTENEDOR - DEJAR QUE TOPBAR ESTÉ VISIBLE -->
	...
	<?php elseif (isset($_SESSION['login_type']) && $_SESSION['login_type'] == 4): // ESTUDIANTE ?>
		<div class="inicio-cards">
			<a href="index.php?page=student_grades" class="inicio-card student-card" data-toggle="tooltip" title="Ver mis calificaciones">
				<i class="fa fa-clipboard-list"></i>
				<span>Mis Notas</span>
			</a>
			<a href="index.php?page=student_attendances" class="inicio-card student-card" data-toggle="tooltip" title="Ver mis asistencias">
				<i class="fa fa-calendar-check"></i>
